a = "І остання, 1385-а перевірка: Solов'їна!"
...
def GetReady(string):
    ...
    is_able_change = True
    i = 0
    # делает распределение типов текста (ЭТАП: ДО ЗАШИФРОВКИ)
    while (i < leng):
        if (ord(string[i]) >= 33 and ord(string[i]) <= 126):

            if (i > 0 and (ord(string[i - 1]) >= 33 and ord(string[i - 1]) <= 126 or ord(string[i - 1]) in (1, 2, 3))):
        ...
        if (ord(string[i]) >= 1040 and ord(string[i]) <= 1070
                or ord(string[i]) >= 1072 and ord(string[i]) <= 1103
                or ord(string[i]) in (1105, 1025)):

            if (i > 0 and (ord(string[i - 1]) >= 1040 and ord(string[i - 1]) <= 1070
                           or ord(string[i - 1]) >= 1072 and ord(string[i - 1]) <= 1103
